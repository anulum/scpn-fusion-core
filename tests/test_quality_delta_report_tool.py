...
# SCPN Fusion Core — Quality Delta Report Tool Tests
"""Behavioural contract tests for ``tools/coverage_delta_report.py``."""
...
def test_strict_mode_writes_passing_machine_and_markdown_reports(tmp_path: Path) -> None:
    """A compliant quality summary produces JSON and Markdown evidence."""
...
def test_strict_mode_fails_when_required_branch_metric_is_missing(tmp_path: Path) -> None:
    """A missing required metric is reported as debt and fails strict mode."""
    summary = tmp_path / "summary.json"
    thresholds = tmp_path / "thresholds.json"
    out_json = tmp_path / "report.json"
    out_md = tmp_path / "report.md"
