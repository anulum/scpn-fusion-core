...
"""Coverage and contract tests for neural-transport math helper functions."""
...
def _mlp_weights(
    *,
    ...
    layers_w: list[FloatArray] = [
        np.full((dims[i], dims[i + 1]), 0.1, dtype=np.float64) for i in range(n_layers)
    ]
    layers_b: list[FloatArray] = [np.zeros(dims[i + 1], dtype=np.float64) for i in range(n_layers)]
    return SimpleNamespace(
        input_mean=np.zeros(n_in, dtype=np.float64),
        input_std=np.ones(n_in, dtype=np.float64),
        n_layers=n_layers,
        layers_w=layers_w,
        layers_b=layers_b,
        gated=gated,
...
class TestActivations:
    ...
    def test_softplus_clipped_extremes(self) -> None:
    ...
    def test_gelu_positive_monotone(self) -> None:
        x = np.linspace(0, 5, 100)
        out = _gelu(x)
        assert np.all(np.diff(out) >= 0)

    def test_gelu_matches_approx(self) -> None:
        x = np.array([1.0])
...
class TestComputeNustar:
    """Electron collisionality helper contracts."""

    def test_iter_baseline(self) -> None:
        nu = _compute_nustar(te_kev=10.0, ne_19=10.0, q=1.5, rho=0.5)
        assert nu > 0
        assert np.isfinite(nu)

    def test_collisionality_increases_with_density(self) -> None:
    ...
    def test_collisionality_decreases_with_temperature(self) -> None:
    ...
    def test_zero_rho_safe(self) -> None:
...
class TestMlpForward:
    """MLP transport-head helper contracts."""

    def test_gated_with_log_transform_and_gyrobohm_batched(self) -> None:
    ...
    def test_scalar_input_uses_scalar_gyrobohm(self) -> None:
        weights = _mlp_weights(
            gated=True,
            log_transform=False,
            gb_scale=True,
    ...
    def test_ungated_plain_softplus_output(self) -> None:
        weights = _mlp_weights(
            gated=False,
            log_transform=False,
            gb_scale=False,
