...
def lif_step(v, z, x, weights, bias, tau_rc, dt):
    ...
    # determine spike
    z_next = jax.lax.stop_gradient(spike_function(v_next)) + (surrogate_spike(v_next) - jax.lax.stop_gradient(surrogate_spike(v_next)))
...
def generate_frc_data(n_samples: int, grid_size: int, seed: int = 42):
    ...
    for _ in range(n_samples):
        inputs = RigidRotorFRCInputs(
            ...
            T_e_eV=rng.uniform(2000, 10000), theta_dot=0.0,
            R_s=rng.uniform(0.15, 0.3), B_ext=rng.uniform(2.0, 8.0),
