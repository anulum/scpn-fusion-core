...
REQUIRED_ITEMS = (
    ...
    "Internal readiness register regenerated in current branch",
...
def _parse_readiness_state(text: str) -> str | None:
    match = re.search(r"(?mi)^Readiness State:\s*`?([^`\r\n]+)`?\s*$", text)
    return match.group(1).strip() if match else None
...
def check_release_readiness(
    ...
    *,
    ...
    """Parse release readiness state and return structural violations as errors."""
    errors: list[str] = []
    if not readiness_path.exists():
        return [f"Release readiness file missing: {readiness_path}"]
    ...
    if not release_version:
        ...
        errors.append(
            f"Readiness release version mismatch: expected {expected_version}, "
            f"found {release_version}."
        )

    state = _parse_readiness_state(text)
    if require_ready_state:
        if not state:
            errors.append("Missing 'Readiness State:' line.")
        elif state.lower() != "ready":
            errors.append(f"Readiness state must be 'ready' (found: {state}).")

    parsed_items = _parse_check_items(text)
    for required in REQUIRED_ITEMS:
        key = _normalize(required)
        if key not in parsed_items:
            errors.append(f"Missing readiness item: {required}")
            continue
        if not parsed_items[key]:
            errors.append(f"Readiness item is not checked: {required}")
    return errors


def main(argv: list[str] | None = None) -> int:
    """Validate readiness consistency and return non-zero on any acceptance failure."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--readiness-file",
        default=str(DEFAULT_READINESS_FILE),
        help="Path to release readiness markdown.",
    )
    parser.add_argument(
        "--expected-version",
        default=None,
    ...
    parser.add_argument(
        "--allow-not-ready",
        action="store_true",
        help="Allow readiness state other than 'ready'.",
    )
    ...
    if errors:
        print(f"Release readiness FAILED ({len(errors)} issue(s))")
        for error in errors:
            print(f" - {error}")
        return 1

    print(f"Release readiness passed: {readiness_path.relative_to(REPO_ROOT).as_posix()}")
    return 0
