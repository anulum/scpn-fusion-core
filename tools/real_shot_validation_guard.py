...
def evaluate(*, report: dict[str, Any], thresholds: dict[str, Any]) -> dict[str, Any]:
    ...
    ----------
    report
        Report payload containing equilibrium, transport, and disruption sections.
    thresholds
        Guard configuration including minima, required machines, and source-contract flags.

    Returns
    -------
    dict[str, Any]
        Per-domain verdicts and aggregate pass status.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    ----------
    argv
        Optional CLI argument list. If omitted, use process arguments.

    Returns
    -------
    int
        ``0`` when the guard passes, or ``1`` when a criterion fails.

    Raises
    ------
    ValueError
        If a report or threshold payload is malformed.
    FileNotFoundError
