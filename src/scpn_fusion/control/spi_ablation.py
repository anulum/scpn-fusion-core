...
class SpiAblationSolver:
    def __init__(
        ...
        for i in range(n_fragments):
            ...
            self.fragments.append(SpiFragment(id=i, radius=r_frag, mass=m_frag, pos=pos, vel=vel))
    ...
    def step(
        ...
        for frag in self.fragments:
            ...
            rho_loc = np.sqrt(((r_loc - R_MAJOR) / A_MINOR) ** 2 + (z_loc / ELONGATION) ** 2)
            ...
            T_e = np.interp(
                rho_loc, np.linspace(0, 1, len(plasma_te_profile)), plasma_te_profile
            )  # [keV]
