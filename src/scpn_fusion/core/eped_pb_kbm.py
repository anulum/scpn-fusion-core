...
Two ballooning geometries are selectable via ``geometry``. The default
``"salpha"`` evaluates the boundary in reduced circular s-alpha geometry
(first stability only), which collapses for strongly shaped pedestals because
it lacks second-stability access. The ``"miller_second_stability"`` geometry
instead reads the committed pyrokinetics reference
(:class:`scpn_fusion.core.ballooning_second_stability.BallooningSecondStabilityReference`,
generated by :mod:`validation.ballooning_reference_runner`), which resolves the
shaped-geometry second-stability access that real pedestals exploit.

Honest scope boundary: even the shaped path is an infinite-n *ideal* ballooning
boundary coupled to the published elliptical peeling proxy — it is NOT the full
ELITE/EPED peeling-ballooning eigenvalue. The peeling coupling remains the
elliptical proxy. The width-height scaling tier
...
_SALPHA = "salpha"
_MILLER_SECOND_STABILITY = "miller_second_stability"
_GEOMETRIES = (_SALPHA, _MILLER_SECOND_STABILITY)
...
class PBKBMPedestalModel:
    ...
    def __init__(
        ...
        A_ion: float = 2.0,
        Z_eff: float = 1.5,
        geometry: str = _SALPHA,
        ballooning_reference: BallooningSecondStabilityReference | None = None,
    ) -> None:
        ...
        geometry_norm = geometry.strip().lower()
        if geometry_norm not in _GEOMETRIES:
            raise ValueError(f"geometry must be one of {_GEOMETRIES}, got {geometry!r}")
        self.geometry = geometry_norm
        self._ballooning_reference: BallooningSecondStabilityReference | None
        if self.geometry == _MILLER_SECOND_STABILITY:
            self._ballooning_reference = (
                ballooning_reference
                if ballooning_reference is not None
                else BallooningSecondStabilityReference.from_artifact()
            )
        else:
            if ballooning_reference is not None:
                raise ValueError(
                    "ballooning_reference is only used by the "
                    f"{_MILLER_SECOND_STABILITY!r} geometry"
                )
            self._ballooning_reference = None
    ...
    def ballooning_alpha_crit(self) -> float:
        """Ballooning-critical alpha at the pedestal shear.

        For the ``salpha`` geometry this solves the infinite-n ideal-MHD
        ballooning equation in reduced s-alpha geometry by bisection over alpha
        at fixed pedestal shear (first-stability boundary only). For the
        ``miller_second_stability`` geometry it interpolates the committed
        pyrokinetics reference, which resolves the shaped-geometry
        second-stability access that lifts ``alpha_crit`` (or removes the
        ballooning cap entirely) at the pedestal.

        Returns
        -------
        float
            Critical normalised pressure gradient at the pedestal shear.
        """
        s_ped = self.pedestal_shear()
        if self.geometry == _MILLER_SECOND_STABILITY:
            assert self._ballooning_reference is not None  # set for this geometry
            alpha_crit = self._ballooning_reference.alpha_crit_at(s_ped)
        else:
    ...
    def predict(
        ...
        *,
        ...
        return PBKBMPedestalResult(
            ...
            method=f"pb_kbm_{self.geometry}",
        )
    ...
    *,
    ...
    if tier_norm == "pb_kbm":
        model = PBKBMPedestalModel(
            ...
            Z_eff=Z_eff,
            geometry=geometry,
            ballooning_reference=ballooning_reference,
        )
