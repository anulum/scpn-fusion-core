...
- Enforce complete public API documentation in `fusion-gpu`,
  ...
  `fusion-control`, `fusion-core`, `fusion-physics`, and `scpn-fusion-rs` PyO3
  crates; document their grid,
...
- The predictive IDA boundary estimator now uses a differentiable sub-cell
  ...
  improves ψ_N RMSE from `0.3077836366` to `0.2109971505` and raw
...
- Return typed shape/data errors from `fusion-ml` PCA, neural-equilibrium, and
  PCE convenience APIs, and enforce non-test panic-API denial across the crate.
