...
| Python capability source modules | 321 |
...
| Implicit-IDA same-case and stationarity evidence | `blocked_same_case_evidence`; current-runtime public 129² DIII-D-like FreeGS comparison has ψ_N RMSE `0.2110185203` and non-isolated warm P95 `30.8563 ms`. The completed `33/65/129/257` coil-vacuum diagnostic reproduces the exact 129 anchor and finds second-order source-free forcing convergence (`2.0120`, `2.0026`), but current recovery is non-monotone and the finest source-free response changes by relative L2 `0.67449`; routing is therefore `mixed_source_and_vacuum_error`, with every facility, control, safety, scientific, latency, and held-out claim still false | `validation/benchmark_ida_same_case.py`, `validation/reports/ida_same_case_evidence.md`, `validation/reports/ida_fixed_point_stability.md`, `validation/reports/ida_operator_response.md`, `validation/reports/ida_coil_vacuum_grid_convergence.md` |
