...
def _run_case(nr: int, nz: int, coeff: float) -> dict[str, Any]:
    r_min, r_max = 1.0, 3.0
    z_min, z_max = -1.0, 1.0
...
def _write_markdown(report: dict[str, Any]) -> None:
    ...
    for case in report["cases"]:
        lines.append(
            f"| {case['nr']}x{case['nz']} | {case['coeff']:.6g} | "
            f"{case['elapsed_s']:.6e} | "
...
def main() -> int:
    ...
    cases = [_run_case(17, 19, -0.25), _run_case(33, 35, -0.125)]
    passed = all(
