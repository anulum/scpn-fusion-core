...
def run_campaign(
    *,
    ...
    """Run the Task 13 free-boundary disruption policy recovery scenario.

    This executes the supervisory simulation with disturbance and recovery phases,
    captures alert-mode transitions, and evaluates deterministic policy recovery
    metrics against configured pass thresholds.

    Returns
    -------
    dict[str, Any]
        Scenario metadata, policy closed-loop metrics, recovery-window diagnostics and
        threshold outcomes.
    """
...
def generate_report(**kwargs: Any) -> dict[str, Any]:
    """Create a timestamped Task 13 report envelope.
...
def render_markdown(report: dict[str, Any]) -> str:
    """Render Task 13 report data as markdown."""
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point for Task 13 disruption policy recovery validation.

    Returns
    -------
    int
        ``0`` for pass or non-strict mode, ``2`` when strict mode fails.
    """
    ...
    parser.add_argument(
        "--output-json",
        default=str(
            ROOT / "validation" / "reports" / "task13_free_boundary_disruption_policy_recovery.json"
        ),
    )
    parser.add_argument(
        "--output-md",
        default=str(
            ROOT / "validation" / "reports" / "task13_free_boundary_disruption_policy_recovery.md"
        ),
    )
