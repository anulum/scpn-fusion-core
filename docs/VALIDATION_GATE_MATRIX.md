...
| `freegs-strict` (`freegs-strict.yml`, manual dispatch) | Strict FreeGS backend parity lane; fails on any fallback, non-FreeGS reference backend, or unconverged case. | `freegs-strict` |
...
enforces no-fallback and all-cases-converged contracts when invoked.
