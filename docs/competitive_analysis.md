...
|------|-------------|-------------|----------|--------|
| **SCPN v3.10.1 (Rust reduced-order kernel)** | **~2 MHz** | **0.52 us P50 / 0.70 us P99** | Rust | Committed Rust-extension campaign artifact |
| **SCPN v3.10.1 (controller loop, surrogate plant)** | **>10 kHz** | **PID p95 0.012 ms; SNN p95 0.284 ms** | Python | `validation/reports/scpn_end_to_end_latency.md` |
| **SCPN v3.10.1 (controller loop, full-mode plant)** | **>10 kHz** | **PID p95 0.047 ms; SNN p95 0.294 ms** | Python | `validation/reports/scpn_end_to_end_latency.md` |
...
| **SCPN v3.10.1 (1.5D step)** | **1.5D coupled** | **1.5--5.5 us per step** | Crit-gradient + neoclassical | CI Criterion |
| **SCPN v3.10.1 (MLP)** | Neural surrogate | **~24 ns single-point** | Trained QLKNN-10D surrogate | CI Criterion |
| **SCPN v3.10.1 (native GK)** | Linear eigenvalue | **~0.3 s/surface** | Ballooning, Sugama collisions | Measured |
...
| **SCPN v3.10.1 (Rust)** | 65x65 | Picard + SOR | **~100 ms** | Measured |
| **SCPN v3.10.1 (Neural)** | 129x129 | PCA + MLP surrogate | Latency not public-claimed in this release | Tracked surrogate artifact |
| **SCPN v3.10.1 (Multigrid)** | 65x65 | V-cycle | **~15 ms** | Projected |
