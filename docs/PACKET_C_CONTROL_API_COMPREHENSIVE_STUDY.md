...
> "Packet C — `scpn/runtime.py` — PetriNetEngine (future integration): The runtime engine wraps the `CompiledNet` in a simulation loop."
...
```
        ...
        (f, m')_oracle  (f, m')_sc
              ...
              └───────┬───────┘
                      │
            commit SC state (or oracle reference state)
                      │
               decode_actions()
                      │
                  ControlAction
...
The oracle path is always available and deterministic. The SC path currently returns the oracle result as a reference-mode implementation, but is structured so that when the Rust Petri kernel is exposed, `_sc_step()` becomes a real stochastic execution and the dual-path architecture enables online comparison:
...
Malformed artifact JSON is now covered through strict loader/schema guard tests. The untested lines in `controller.py` are the `_sc_step` method's body (currently a one-line oracle reference path), which will be exercised when the Rust SC kernel is integrated.
...
3. **Schema validation.** The loader now applies strict required-key and closed-object schema checks before dataclass construction, backed by malformed-artifact regression tests. Optional third-party JSON Schema conformance testing remains outside the runtime dependency set.
...
Online learning is deferred outside the current certified runtime scope; Section 34 records the safety constraints that would be required before reconsideration (bounded weight deltas, Lyapunov stability monitoring).
...
Plant model integration is deferred outside the current portable controller scope; Section 31 records the coupling requirements for any later machine-specific extension.
...
Hierarchical multi-controller composition is deferred outside the single-rate artifact contract; Section 33 records the scheduler and determinism requirements for any later orchestration extension.
...
4. **Divergence policy.** If the divergence exceeds a threshold, the controller should select the deterministic oracle reference path. This provides a safety net during the SC path integration period.
...
5. **Schema validation scope.** Runtime loading now rejects missing required keys and unknown fields before dataclass construction. Full external JSON Schema conformance testing remains a release-validation concern rather than a runtime dependency.
