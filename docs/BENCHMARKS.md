...
`scpn-fusion repro --full` is the one-command wrapper over the tracked
fail-closed chain. It reruns `validation/full_fidelity_end_to_end_campaign.py`,
refreshes the public ledger, and writes
`validation/reports/full_reproduction_evidence.json` plus
...
keeps publication readiness false while any full-fidelity lane is blocked.
full-reproduction evidence is generated by `scpn-fusion repro --full` and
tracked in `validation/reports/full_reproduction_evidence.md`; it carries the
same fail-closed boundary plus per-report SHA-256 checksums for audit replay.
Public
