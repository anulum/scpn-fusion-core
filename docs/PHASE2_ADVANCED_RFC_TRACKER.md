...
| GPHY-03 | Done | Done (`docs/rfc/GPHY-03_RFC.md`, `scpn-fusion-rs/crates/fusion-nuclear/src/wall_interaction.rs`) | Done (synthetic ADAS-style table scope) | Done (charge-state PEC differentiation + CR power/loss checks) | Done (`cargo test -p fusion-nuclear` bounded) | Unassigned |
| GPHY-04 | Done | Done (`docs/rfc/GPHY-04_RFC.md`, `scpn-fusion-rs/crates/fusion-math/src/iga.rs`, `scpn-fusion-rs/crates/fusion-nuclear/src/wall_interaction.rs`) | Done (synthetic NURBS control-point scope) | Done (NURBS closure + endpoint + envelope regression checks) | Done (`cargo test -p fusion-math`, `cargo test -p fusion-nuclear`) | Unassigned |
| GPHY-05 | Done | Done (`docs/rfc/GPHY-05_RFC.md`, `scpn-fusion-rs/crates/fusion-control/src/digital_twin.rs`, `scpn-fusion-rs/crates/fusion-control/src/mpc.rs`) | Done (synthetic stochastic-latency scope) | Done (vector OU determinism + delay-line + lag-aware MPC checks) | Done (`cargo test -p fusion-control` bounded) | Unassigned |
| GPHY-06 | Done | Done (`docs/rfc/GPHY-06_RFC.md`, `scpn-fusion-rs/crates/fusion-core/src/jit.rs`) | Done (synthetic reduced runtime-specialization scope) | Done (regime routing + compile-cache reuse + hot-swap response checks) | Done (`cargo test -p fusion-core`, `cargo clippy -p fusion-core --all-targets --all-features -- -D warnings`) | Unassigned |
