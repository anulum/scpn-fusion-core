...
\begin{enumerate}
  ...
  \item A \textbf{three-packet compilation pipeline} (Sections~\ref{sec:compiler}
    and~\ref{sec:controller}) that maps places to neuron populations,
...
\begin{table}[htbp]
\centering
\caption{Controller latency comparison.  SNN latency on FPGA is the
projected per-spike time for a Xilinx Alveo U250 operating at
...
Since v2.0.0, the Rust SNN implementation has been exposed to Python
...
The disruption predictor has been recalibrated to achieve 100\% recall
with 0\% false positive rate on the 16-shot DIII-D reference dataset,
and a Riccati-based $H_\infty$ controller provides robust vertical
stability synthesis.
...
\url{https://github.com/anulum/scpn-fusion-core}, release v2.1.0.  The
