...
def main():
    ...
    for name, pos in sensor_places.items():
        _draw_place(ax, pos, sensor_labels[name], color=sensor_color,
                    edgecolor=COLORS["blue"])

    # Draw action places (red tint)
    action_color = '#FADBD8'
    action_labels = {
        'coil_up': r'$I_{\mathrm{coil}}^{+}$',
        'coil_dn': r'$I_{\mathrm{coil}}^{-}$',
        'heat_up': r'$P_{\mathrm{heat}}^{+}$',
        'heat_dn': r'$P_{\mathrm{heat}}^{-}$',
    }
    for name, pos in action_places.items():
        _draw_place(ax, pos, action_labels[name], color=action_color,
                    edgecolor=COLORS["red"])

    # Draw transitions (dark bars)
    trans_labels = {
        'T_corr_up': r'$T_{\rm up}$',
        'T_corr_dn': r'$T_{\rm dn}$',
        'T_heat_up': r'$T_{h+}$',
        'T_heat_dn': r'$T_{h-}$',
    }
    ...
    # Column labels
    ax.text(sensor_x, 4.16, 'Sensor places\nobservations',
            ha='center', va='bottom', fontsize=7.5, fontweight='bold',
            color=COLORS["blue"])
    ax.text(trans_x, 4.16, 'Transitions\nfiring gates',
            ha='center', va='bottom', fontsize=7.5, fontweight='bold',
            color='0.25')
    ax.text(action_x, 4.16, 'Action places\ncommands',
            ha='center', va='bottom', fontsize=7.5, fontweight='bold',
            color=COLORS["red"])
