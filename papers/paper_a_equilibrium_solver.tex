...
Metal/D3D12 abstraction layer for cross-vendor GPU portability.  Projected
speedups on RTX 4090-class hardware are $20{-}50\times$ for
...
Since the initial release, v2.1.0 has added:
\begin{itemize}
  \item A self-consistent GS$\leftrightarrow$transport outer iteration
    loop (\code{run\_self\_consistent()}) that alternates Grad--Shafranov
    solves with 1-D transport evolution until $\Psi$ convergence.
  \item Five MHD stability criteria (Mercier, first-stability ballooning,
    Kruskal--Shafranov, Troyon $\beta$ limit, NTM seeding threshold).
  \item Calibrated $\beta_N$ estimation achieving $<3\%$ error against
    ITER and SPARC design points.
  \item A FreeGS blind benchmark with Solov'ev analytic comparison
    on three test cases (ITER-like, SPARC-like, spherical tokamak).
\end{itemize}
...
\begin{itemize}
  ...
  \item MPI domain decomposition for high-resolution ($512 \times 512$
    and above) equilibria on leadership computing facilities.
  \item Training the neural surrogate on the full SPARC scenario library
    to achieve $<5\%$ relative $L^2$ error.
  \item Monte Carlo uncertainty propagation through the full
...
licence.  The version used in this work corresponds to release v2.1.0.
