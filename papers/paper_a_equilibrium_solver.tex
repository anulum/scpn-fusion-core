...
\begin{itemize}
  \item Deployment of the multigrid V-cycle as the default inner solver,
    which is expected to reduce reconstruction time by a factor of
    $3{-}5\times$.
  ...
  \item Extension of the validation database to include DIII-D and JET
    GEQDSK files for cross-machine validation.
  \item Training the neural surrogate on the full SPARC scenario library
    to achieve $<5\%$ relative $L^2$ error.
  \item Point-wise $\Psi$ RMSE validation against reference equilibria,
    going beyond the current global-scaling and axis-position metrics.
\end{itemize}
...
licence.  The version used in this work corresponds to release v1.0.2.
